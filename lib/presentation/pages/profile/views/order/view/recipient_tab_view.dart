...
class RecipientTabView extends StatefulWidget {
  const RecipientTabView({
    super.key,
  });

  @override
  State<RecipientTabView> createState() => _RecipientTabViewState();
}

class _RecipientTabViewState extends State<RecipientTabView>
  ...
  void initState() {
    ...
    cubit.getOrder(1, option: ShipmentOption.recipient);
  ...
  void getOrder({bool? isRefresh = false}) {
    cubit.getOrder(
      currentPage,
      option: ShipmentOption.recipient,
      ...
      child: BlocConsumer<OrderCubit, OrderState>(
        listener: (context, state) {
          if (state is OrderLoaded) {
            ...
            totalPage = state.model.totalPages ?? 1;
