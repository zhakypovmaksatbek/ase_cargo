...
class FormCubit extends Cubit<FormCubitState> {
  ...
  Future<void> saveForm() async {
    ...
    try {
      await repo.saveForm();
      await FormDioSettings().clearCookies();
  ...
  Future<void> createForm(String userRole) async {
    emit(FormLoading());
    try {
      final result = await repo.createForm(userRole);
      emit(FormSuccess(formMissingStepModel: result, steps: FormSteps.first));
    ...
    try {
      ...
      emit(
        FormError(
          errorDetail: e.response?.data['detail'] ??
              LocaleKeys.exception_something_went_wrong_try_again.tr(),
          senderErrorModel: SenderErrorModel.fromJson(e.response?.data),
        ),
      );
    }
  }
    ...
    try {
      ...
      emit(
        FormError(
          errorDetail: e.response?.data['detail'] ??
              LocaleKeys.exception_something_went_wrong_try_again.tr(),
          recipientErrorModel: SenderErrorModel.fromJson(e.response?.data),
        ),
      );
    }
  }
    ...
    try {
      ...
      emit(
        FormError(
          errorDetail: e.response?.data['detail'] ??
              LocaleKeys.exception_something_went_wrong_try_again.tr(),
          additionsErrorModel: AdditionsErrorModel.fromJson(e.response?.data),
        ),
      );
    }
  }
    ...
    try {
      ...
      emit(
        FormError(
          errorDetail: e.response?.data['detail'] ??
              LocaleKeys.exception_something_went_wrong_try_again.tr(),
          packageErrorInfoModel:
              PackageErrorInfoModel.fromJson(e.response?.data),
        ),
      );
    }
  }
